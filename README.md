...
如果你的游戏本体已更新到0.5.6.x版本，请下载[0.08-for-dol-0.5.8](https://github.com/ZeroRing233/Degrees-of-Lewdity-RobinMod/releases/tag/0.08-for-dol-0.5.8)中的domRobin.mod.zip(请不要下载source code)
...
#### 问题修复
1. 修复了在罗宾房间放了衣服的情况下，pc依旧可以果着直播的问题。
2. 修复了罗宾的存款可能会变成多位小数的问题。

#### 致谢
1. 感谢群友Dusty为气球摊合作/竞争线提供的进线文本和条件设想。

#### 题外话
1. 055本体更新后，如果选择了艾弗里豪宅线，日志中关于反抗线/被守护线的信息会被原版隐藏，但特质的获取**不会**受到影响。（~~当然，剧情违和感还是避免不了的~~）
2. 如无志愿者帮忙更新或原版内容冲突等情况，本模组下次更新将在明年春节之后。（~~因为作者要准备开新书了，逃~~）（~~等我不菜了之后再考虑两手抓吧TAT~~）
...
关于新规则：根据孤儿院的希望计算，如果希望为满值50，必定有一人计算天数还钱。如果希望大于30，50%概率开始计算天数还钱。如果希望大于15，5%概率计算天数还钱。
3. 罗宾救助/回赠孤儿事件优化。
4. 罗宾柠檬水摊许可证剧情优化。
...
### 0.06-beta
(过渡小版本，主要对0.06-alpha进行了细节修复)
#### 新增功能
1. 在卧室「查看照片集」下方新增了「编辑照片集」选项，支持照片的拖动排序和删除操作。
2. 游戏加载时弹出的提示窗口现可勾选「不再显示此提示」。

#### Bug修复
1. 修复了照片集位置与模拟人生0.77及之前版本的兼容性问题。
2. 修复了荒原观星战斗中偶尔出现的无法结束战斗的bug。
3. 修复了柠檬水摊/巧克力摊在升级前错误显示升级后才会出现的选项问题。

#### 代码修改备注（普通玩家无需关注）
1. 尝试使用webpack打包（目前仅应用于照片集功能），如果想查看照片集相关代码，请直接下载源码，release里是编译后的版本。
