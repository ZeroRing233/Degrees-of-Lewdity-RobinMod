...
本mod基于0.4.7.5版本进行自测（之后会适配0.5.0.5)，更老的版本可能会有不兼容的问题。

安装方式：下载release下0.0.2-alpha中的domRobin.mod.zip，用平时玩游戏用的mod管理器加载完汉化mod后，再加载此mod，安装前记得保存自己的存档码，以防止出现bug无法恢复。

备注：0.0.2-alpha为适配0.4.7.5的最后一个版本，接下来的更新将适配0.5.0.5，新旧版本并不兼容，且旧版本不会再维护, 见谅！（0.0.2-alpha的具体内容为[目前进度](#目前进度) 中的1-14）
